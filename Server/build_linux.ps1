# Build and run Docker container script for Linux testing
...
Write-Host "Build complete! Run the container with: docker run -it --rm -p 33334:33334 tank-server-linux"
$runContainer = Read-Host "Do you want to run the container now? (y/n)"
if ($runContainer -eq "y") {
    Write-Host "Running container..."
    docker run -it --rm -p 33334:33334 tank-server-linux
}
else {
    Write-Host "You can manually run the container with: docker run -it --rm -p 33334:33334 tank-server-linux"
} 